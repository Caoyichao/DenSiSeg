...
def get_points(mat, lab, k):
    ...
    while i < k or j < k:
        ...
        if lab[h, w] == 1:    # positive
            ...
            res_labs.append(0.)
            j += 1
        res_points.append(mat[h, w])
        flag += 1
        if flag >= 1000:
            break
    return res_points, res_labs
...
k = 10  # 每个图像上随机选择k个数据点入库
all_points = []
all_points_labels = []

for i, img_path in enumerate(tqdm.tqdm(images[:100])):
    image = cv2.imread(img_path)
    lab_path = img_path.replace('blendall','gt_blendall').replace('jpg','png')
    #lab = cv2.imread(labels[i], 0)
    lab = cv2.imread(lab_path, 0)
    lab = np.where(lab==255, 1, 0)
    ...
    mat = seg_output.squeeze(0).detach().cpu().numpy()
    mat = mat.transpose(1,2,0)
    points, sub_labs = get_points(mat, lab, k)
    all_points.extend(points)
    all_points_labels.extend(sub_labs)
...
mat = np.zeros((nums, 1))
for i in range(nums):
    mat[i] = all_points_labels[i]
np.save('labels.npy', mat)
